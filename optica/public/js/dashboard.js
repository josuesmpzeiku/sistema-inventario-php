        ...
        item.addEventListener('click', async function () {
            ...
            if (moduloSeleccionado === 'Admisión') {
                try {
                    ...
                    if (
                        ...
                        scriptAdm.src =
                            `${BASE_URL}/js/admision.js`;

                        scriptAdm.onload = () => {
                            if (
                                typeof inicializarModuloAdmision ===
                                'function'
                            ) {
            ...
            if (moduloSeleccionado === 'Consultorio') {
                try {
                    ...
                    scriptCons.src =
                        `${BASE_URL}/js/consultorio.js?v=${Date.now()}`;

                    scriptCons.onload = () => {
                        if (
                            typeof inicializarModuloConsultorio ===
                            'function'
                        ) {
            ...
            if (moduloSeleccionado === 'Ventas') {
                try {
                    ...
                    scriptVen.src =
                        `${BASE_URL}/js/ventas.js?v=${Date.now()}`;

                    scriptVen.onload = () => {
                        if (
                            typeof inicializarModuloVentas ===
                            'function'
                        ) {
    ...
    if (btnLogout) {
        btnLogout.onclick = async () => {
            ...
            try {
                ...
                console.error(
                    'Fallo crítico en cierre de sesión:',
                    error
                );
            }
        };
