...
<html>
    ...
    <body>
        <div class="color-arriba"><h2 class ="subtitulo">SEGUIMIENTO Y FICHA DE PACIENTES</h2></div>  
        <div class="index-form-login">          
            <button id="bt-div-1" name = "bt-div-1" class="btn-universal"><span class="icon icon-search"></span></button>
        </div>       
        <section id="div1">
                     <?php
                $sql11 ="SELECT atencion.idatencion, atencion.fecatencion, paciente.nompaciente, paciente.idpaciente, atencion.estatencion 
                ...
                while ($row11 = mysqli_fetch_array($resultado11)){                     
                    print "<button class='btn-listado'>{$row11['idatencion']},   {$row11['nompaciente']},   {$row11['fecatencion']},   {$row11['estatencion']}</button>";                                 
          ...
          <form name="form" method="post" action=""> 
                <div class="fila">
                    <div class="columna">
                        <table width="100%">
                            <tr>
                                <td><input class="decorar-input" name="bpaciente" type="text" id="bpaciente" placeholder="Escriba un Nombre de Paciente para buscar" required><div id="npaciente"></div></td>
                                <td width="5%"><button type="submit" name="sela" class="btn-universal" id="sela"><span class="icon icon-folder-download"></span></button></td>
                            <tr>
                        </table>    
                    </div>                
                </div>
            ...
            if (isset ($_POST['sela'])){
                ...
                $sql1="SELECT paciente.*, TIMESTAMPDIFF(YEAR,fecnpaciente,CURDATE()) AS edad FROM paciente WHERE idpaciente = '".$idpaci."'";
                $resultado1 = mysqli_query($conn, $sql1);
                if ($row1 = mysqli_fetch_row($resultado1)){ }
                print "<div class='fila'>";                          
                    print "<table class = 'TablaB'>";                                   
                ...
                $sql11 ="SELECT * FROM atencion WHERE idpaciente = '".$idpaci."'";
                $resultado11 = mysqli_query($conn, $sql11);
                while ($row11 = mysqli_fetch_array($resultado11)){  
