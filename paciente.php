...
<html>
    ...
    <body>
        <div class="color-arriba"><h2 class ="subtitulo">PACIENTES POR ATENDER</h2></div><br> 
        <?php
            $sql1 ="SELECT atencion.*, paciente.*
                    FROM atencion INNER JOIN paciente ON paciente.idpaciente = atencion.idpaciente
                    WHERE atencion.idtienda = ".$tienda." AND atencion.estatencion = 'RECEPCION'";
            $resultado1 = mysqli_query($conn, $sql1);
            while ($row1 = mysqli_fetch_array($resultado1)){  
                print "<a class='tm-btn-plus' href='cuestionario.php?perfil=".$ussera."&atencion={$row1['idatencion']}'>{$row1['idatencion']},   {$row1['nompaciente']},   {$row1['fecatencion']}</a><br>";                              
            }
        ?>
