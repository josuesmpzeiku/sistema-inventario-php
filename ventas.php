...
<html>
    ...
    <body>
        ...
        <?php
           /* $sql2 ="SELECT SUM(totalventa) FROM `venta` INNER JOIN atencion ON atencion.idatencion = venta.idventa
                    WHERE atencion.idtienda = '".$tienda."' AND MONTH(CURDATE())=MONTH(venta.fecventa);";
            ...
            while ($row1 = mysqli_fetch_array($resultado1)){  
                print "<a class='tm-btn-plus' href='detalleventa.php?perfil=".$ussera."&iventa=".$row1['idventa']."'>{$row1['idventa']},   {$row1['nompaciente']},   {$row1['fecventa']},     Q {$row1['totalventa']}</a><br>";                              
            }
        ?>
